...
import (
	...
	// "github.com/tools/jot/repository"
	// "github.com/nox/util"
)
...
type JotRestService struct {
	
}

//NewSobarRestService retuens a new initialized version of the service
func NewJOTRestService(config []byte, verbose bool) *JotRestService {
	service := new(JotRestService)
...
func (jotRestService *JotRestService) Init(config []byte, verbose bool) error {
	if verbose {
...
func (jotRestService *JotRestService) Serve(address string, port int, stopSignal chan bool) {
